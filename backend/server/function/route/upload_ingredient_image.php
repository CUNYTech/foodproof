<?php
$app->post('/ingredient/upload_picture', function($request, $response, $path = null) {
        ...
        if(isset($_FILES['image'])) {
            ...
            if($image_data){
                ...
                if($db){
                    ...
                    if(sizeof($error)==0){
                        add_ingredient_image_data($image_data,$ingredient_id,$db,$error);
                        if(sizeof($error)==0){
                            $out['Upload']='succeed';
                        }
                        else{
                            $error['Upload']='failed';
                            unlink(IMAGE.'/'.$image_data['name']);
                        }
                    }
